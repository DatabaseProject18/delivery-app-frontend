...
const QuarterlyIncome = (props) => {
  ...
  useEffect(async () => {
    ...
    await requestTruckData();
    setLoading(false);
  }, [year,month]);

  const requestTruckData = async () => {
    let res;
    if (year === "All" && month === "All"){
      res = await api.report.getTruckUsedHours();
    }else if (year !== "All" && month !== "All" ){
      res = await api.report.getTruckUsedHours(year,month);
    }else if (year !== "All" && month === "All"){
      res = await api.report.getTruckUsedHours(year);
    }
    if (res.resCode === 200) {
      setTruckData(res.result.data.multiple);
    } else {
  ...
  const fields = [
    { key: "registration_no", label: "Registration Number", _style: { width: "30%" } },
    {
      key: "truck_capacity",
      label: "Truck Capacity (m^3)",
      _style: { width: "20%" },
    },
    { key: "store", label: "Store Name", _style: { width: "20%" } },
    {
      key: "num_of_turns",
