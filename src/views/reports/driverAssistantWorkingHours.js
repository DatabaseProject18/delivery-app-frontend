...
const QuarterlyIncome = (props) => {
  ...
  const [driverAssistantData, setDriverAssistantData] = useState([]);
  ...
  useEffect(async () => {
    ...
    await requestDriverAssistantData();
    setLoading(false);
  }, [year,month]);

  const requestDriverAssistantData = async () => {
    let res;
    if (year === "All" && month === "All"){
      res = await api.report.getDriverAssistantWorkingHours();
    }else if (year !== "All" && month !== "All" ){
      res = await api.report.getDriverAssistantWorkingHours(year,month);
    }else if (year !== "All" && month === "All"){
      res = await api.report.getDriverAssistantWorkingHours(year);
    }
    if (res.resCode === 200) {
      setDriverAssistantData(res.result.data.multiple);
    } else {
